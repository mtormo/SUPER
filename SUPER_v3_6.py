...
def CreateIndex(genome):
    ...
    if not os.path.exists(genome_bwt) and not os.path.exists(genome_fai):
        ##bwa
...
def SamToBam(bam_dir,sam_file,rem_dup,typ,iden):#convert sam to bam, in snps (uniques reads) or svs
    map_list2=[]
    map_list3=[]
    map_list4=[]
    map_list5=[]
    ...
    for rem in glob.glob(bam_dir+'/*.uniq_reads.bam')+glob.glob(bam_dir+'/*.sort.bam')+glob.glob(bam_dir+'/*.sort.pcr_rem.bam')+glob.glob(bam_dir+'/*.sort.bam.bai'):
        os.remove(rem)
...
def main():
    ...
    if glob.glob(args.path+'/*'+fastq):
        ...
        if int(args.analysis) == 1 or int(args.analysis) == 3:
            ...
            typ='SNP'
            SamToBam(bam_dir,sam_file_snp,args.rem_dup,typ,iden)
        ...
        if (int(args.analysis) == 2 or int(args.analysis) == 3) and args.ends == 'p':
            ...
            if not os.path.exists(sv_dir):
                os.makedirs(sv_dir)
            typ='SV'
            SamToBam(bam_dir,sam_file,args.rem_dup,typ,iden)
